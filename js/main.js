        ...
        output+=`
          <div class="col-md-3">
            <div class="well text-center">
              ...
              <h5>${movie.title}</h5>
              <a onclick="movieSelected('${movie.id}')" class="btn btn-primary mb-5" href="#">Movie Details</a>
            </div>
          </div>
        `;
      });
...
function getMovie(){
  ...
  axios.get("https://api.themoviedb.org/3/movie/" + movieId + "?api_key=98325a9d3ed3ec225e41ccc4d360c817")
    ...
    let output = `
        ...
        <div class="row">
          <div class="well" style="margin-right: 20px; margin-left:20px;">
            <h3>Plot</h3>
            ${movie.overview}
            <hr>
            <a href="http://imdb.com/title/${movie.imdb_id}" target="_blank" class="btn btn-primary mb-5">View IMDB</a>
            <a href="index.html" class="btn btn-default mb-5">Go Back To Search</a>
          </div>
        </div>
    `;
    $('#movie').html(output);
